#!/usr/bin/env python3
"""
LaTeX Build System Version 2.0 (5th version)
======================================================

A smart build system that compiles LaTeX documents organized in modules
(e.g., HAI702I_Algebre, HAI703I_Anglais) with caching and dependency 
tracking.
...
TYPICAL PROJECT STRUCTURE:
    src/
        HAIXXXI_YYY/
            ...
            ZZZ/
                ww_main.tex          # Another main file
                ww_1.tex             # Included content
...
USAGE EXAMPLES:
    # Compile changed modules (general case)
    ./compile.py

    # Compile specific module by full name, code or substring
    ./compile.py HAI702I_Algebre
    ./compile.py 702
    ./compile.py Algebre

    # Compile multiple modules (-m or --modules)
    ./compile.py -m 702 705 Algebre

    # Compile range of modules by code (-r or --range)
    ./compile.py -r 702 710

    # Force recompile everything (- f or --force)
    ./compile.py -f
    
    # Clean all build artifacts (-c or --clean)
    ./compile.py -c
    
    # You can combine multiple arguments for more specific cleaning:
    ./compile.py -c -m 702 705        # Clean specific modules
    ./compile.py -c -r 702 710        # Clean range of modules
    ./compile.py -c 703               # Clean specific module by code

    # List detected modules (-l or --list_modules)
    ./compile.py --list_modules

    # Show detailed project info with metrics for fun
    ./compile.py --info
    
    # Get help (-h or --help or -? or --usage)
    ./compile.py -h
"""
...
class Config:
    ...
    COMPILE_TIMEOUT = 180  # max timeout in seconds per compilation
...
class ModuleInfo:
    ...
    name: str
    code: Optional[str]
    valid_identifiers: List[str] = None
    files: List[Path]
    ...
    def __generate_identifiers__(self):
        """Generate valid identifiers for module selection"""
        identifiers = set()
        identifiers.add(self.name)
        if self.code:
            identifiers.add(self.code)
        # Algorithm:
        # HAIXXXI_XX_YY_ZZ
        # remove the prefix part
        # -> XX_YY_ZZ
        # get every single separate part
        # [XX, YY, ZZ]
        # add each part as valid identifier with first letter
        # as caps, no caps, all caps, all no caps
        parts = re.split(r'[_\-]', self.name)
        if len(parts) > 1:
            for part in parts[1:]:
                identifiers.add(part)
                identifiers.add(part.lower())
                identifiers.add(part.upper())
        self.valid_identifiers = list(identifiers)
...
def get_dependencies(tex_file: Path) -> Set[Path]:
    ...
    Scans:
    - Goes through the main file
    - Finds every occurence of \input, \include, \includegraphics
    - Gets the name of the file that follows (with relative path)
    - Collects these files as dependencies and goes through the same
    process in them (recursively) up to max depth of 5.
    """
    deps = set()
    
    def _scan_file(file_path: Path, depth: int):
        if depth > 5 or not file_path.exists():
            return
        ...
        try:
            with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                for inc in includes:
                    for ext in ['.tex', '.sty', '.cls', '.tikz']:
                        inc_path = (file_path.parent / inc).with_suffix('.tex')
                        if inc_path.exists() and inc_path not in deps:
                            deps.add(inc_path)
                            _scan_file(inc_path, depth + 1)
                ...
                for img in graphics:
                    ...
                    for ext in ['.png', '.jpg', '.jpeg', '.pdf']:
                        full_img_path = img_path.with_suffix(ext)
                        if full_img_path.exists():
                            deps.add(full_img_path)
                            break
        except IOError:
            pass
    return deps
...
def needs_rebuild(tex_file: Path, pdf_file: Path, cache: BuildCache) -> bool:
    ...
    A file needs to be rebuilt if one of the following conditions is met:
    - The output PDF does not exist.
    - The main .tex file or any of its dependencies have been modified
        since the last build (based on cached modification times).
    """
    ...
    for dep in deps:
        ...
        cached_dep_mtime = cache.get_mtime(dep)
...
def discover_modules(main_files: List[Path]) -> Dict[str, ModuleInfo]:
    """
    Discover all modules from main files. This shouldn't run
    automatically but only after adding a new module.
...
def compile_files_parallel(files: List[Path], num_jobs: Optional[int] = None) -> List[CompileResult]:
    """
    Compile multiple files one at a time.
    ...
    for f in files:
        ...
        if result.success:
            ...
            # on success also delete the associated log files (and folder)
            if result.log_file.exists():
                result.log_file.unlink()
        else:

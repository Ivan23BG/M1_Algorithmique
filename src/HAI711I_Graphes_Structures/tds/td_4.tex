...
\iftoggle{showsolutions}{
	\begin{td-sol}[]\,\\ % 1
        ...
        \begin{figure}[H]
            \CenterFloatBoxes{}
            \begin{floatrow}
                \input{../assets/tikz/td_4_ex_1_2.tex}

                \input{../assets/tikz/td_4_ex_1_3.tex}
            \end{floatrow}
        \end{figure}
...
\iftoggle{showsolutions}{
	\begin{td-sol}[]\,\\ % 3
        ...
        \vspace{0.2cm}
        \input{../assets/tikz/td_4_ex_3_1.tex}
...
\begin{td-exo}[Menger étendu]\,\\ % 6
    Soient \(G = (V,E)\) un graphe \(k\)-sommet-connexe (c'est-à-dire \(\kappa(G) \geq k\)).

    \begin{enumerate}
        \item On ajoute à \(G\) un sommet \(u\) de degré \(k\). Montrer que le graphe \(G'\) ainsi obtenu est encore \(k\)-sommet-connexe.

        \item Pour le reste de l'exercice, on revient au graphe \(G\) de départ. Soient \(X\) et \(Y\) deux sous-ensembles de \(V\) de taille au moins \(k\). Montrer qu'il existe \(k\) chemins sommet-disjoints de \(X\) à \(Y\).

        \item Soit maintenant \(z\) un sommet de \(G\) n'appartenant pas à \(X\), montrer de même que \(G\) contient \(k\) chemins sommet-disjoints (sauf en \(z\)) de \(z\) à \(X\).
    \end{enumerate}
\end{td-exo}
...
\iftoggle{showsolutions}{
	\begin{td-sol}[]\, % 6
		\begin{enumerate}
            \item Soit \(X\) un ensemble de \(k-1\) sommets de \(G'\). Si \(X \subseteq V\), alors \(G' \setminus X\) est connexe car \(G\) est \(k\)-sommet-connexe et \(u\) a au moins un voisin dans \(G' \setminus X\).

            Si \(u \in X\), alors \(X \setminus \{u\}\) est un ensemble de \(k-2\) sommets de \(G\), donc \(G' \setminus X = G \setminus (X \setminus \{u\})\) est connexe car \(G\) est \(k\)-sommet-connexe et \(k-2 < k\).

            \item On rajoute un point \(x\) qui domine \(X\) et un point \(y\) dominé par \(Y\). Alors, par le point précédent, le graphe \(G''\) ainsi obtenu est encore \(k\)-sommet-connexe.
            Donc, par le théorème de Menger, il existe \(k\) chemins sommet-disjoints de \(x\) à \(y\) dans \(G''\).
            En enlevant \(x\) et \(y\) de ces chemins, on obtient bien \(k\) chemins sommet-disjoints de \(X\) à \(Y\) dans \(G\).

            \item On rajoute un point \(x\) qui domine \(X\). Alors, par le point précédent, le graphe \(G''\) ainsi obtenu est encore \(k\)-sommet-connexe.
            Donc, par le théorème de Menger, il existe \(k\) chemins sommet-disjoints de \(z\) à \(x\) dans \(G''\).
            En enlevant \(x\) de ces chemins, on obtient bien \(k\) chemins sommet-disjoints (sauf en \(z\)) de \(z\) à \(X\) dans \(G\).
        \end{enumerate}
	\end{td-sol}
}{}
...
\begin{td-exo}[Plein de cycles]\,\\ % 7
    Soit \(G = (V,E)\) un graphe \(k\)-sommet-connexe, avec \(k\geq 2\).
    \begin{enumerate}
        \item Soient \(x_1\) et \(x_2\) deux sommets distincts de \(G\). Montrer qu'il existe un cycle contenant \(x_1\) et \(x_2\).

        \item On veut généraliser le résultat précédent. Soient \(\{x_1,\ldots,x_k\}\) un ensemble de \(k\) sommets distincts de \(G\). Montrer qu'il existe un cycle de \(G\) contenant tous les \(x_i\) (on pourra considérer un cycle qui contient le plus grand nombre possible de sommets \(x_i\) puis appliquer le résultat de l'exercice précédent).
    \end{enumerate}
\end{td-exo}
...
\iftoggle{showsolutions}{
	\begin{td-sol}[]\, % 7
		\begin{enumerate}
            \item Le graphe \(G\) étant \(k\)-sommet-connexe avec \(k \geq 2\), il est en particulier \(2\)-sommet-connexe.
            Donc, par le théorème de Menger, il existe deux chemins sommet-disjoints entre \(x_1\) et \(x_2\).
            En combinant ces deux chemins, on obtient bien un cycle contenant \(x_1\) et \(x_2\).

            \item Considérons un cycle \(C\) qui contient un nombre maximum de sommets \(\{x_1,\ldots,x_k\}\). On suppose qu'il ne contient pas tous ces sommets (sinon la preuve est triviale). Supposons qu'il contient \(x_1,\ldots,x_i\) avec \(i<k\).

            Si \(C = x_1,\ldots,x_i\) alors on applique Menger entre \(x_{i+1}\) et \(C\) pour obtenir \(i\) chemins sommet-disjoints entre \(x_{i+1}\) et \(C\). Alors, \((C - x_1 x_2) \cup P_1 \cup P_2\) est un cycle contenant \(x_1,\ldots,x_{i+1}\).

            Sinon, \(C\) contient au moins \(i+1\) sommets. On applique Menger entre \(x_{i+1}\) et \(C\) pour obtenir \(i+1\) chemins sommet-disjoints entre \(x_{i+1}\) et \(C\). Par le principe des tiroirs, au moins deux de ces chemins aboutissent entre deux sommets consécutifs de \(C\). En remplaçant le segment de \(C\) entre ces deux sommets par les deux chemins, on obtient un cycle contenant \(x_1,\ldots,x_{i+1}\).
        \end{enumerate}
	\end{td-sol}
}{}

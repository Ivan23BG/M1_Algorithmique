...
\iftoggle{showsolutions}{
	\begin{td-sol}[]\ % 3
		...
		\begin{enumerate}
			...
			\item%\label{log:td2:ex3} 
			On a \(T \cup \{ F_{K_1}, \ldots, F_{K_m} \}\) avec \(m\) un entier quelconque et \(K\) une constante.
			Alors 
			\begin{equation*}
			...
			\item \(T \cup \{ F_n,n\in \bb N\}\). Par la question précédente, %\~\ref{log:td2:ex3}
			toute partie finie de \(T \cup \{F_n\}\) a un modèle, donc \(T \cup \{F_n\}\) a un modèle \(M\). Alors 
			\begin{equation*}
				M = \left(
					\bb N, <_{\bb N}, S_{\bb N}, a, b
				\right)

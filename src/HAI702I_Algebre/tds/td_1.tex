...
\iftoggle{showsolutions}{
	\begin{td-sol}[]\ %
		...
		\begin{itemize}
			...
			\item Deux vecteurs \(u\) et \(v\) sont colinéaires s'il existe \(\lambda \in \bb R^*\) tel que \(u = \lambda v\).
		\end{itemize}

		On veut former une base \((u, v, w)\) avec les critères ci-dessus.
		...
		\end{equation*}
		Pour que la base soit directe, on veut que \(u\) soit de norme \(1\):
		\begin{equation*}
			\begin{aligned}
				\nn{u} = 1 
				& \iff \sqrt{\sum_{i=1}^3 u_i^2} = 1\\
				& \iff \sqrt{\lambda^2(1^2 + 2^2 + 2^2)} = 1\\
				& \iff \sqrt{9\lambda^2} = 1\\
				& \iff 3\lambda = 1\\
				& \iff \lambda = \frac{1}{3}.
			\end{aligned}
		\end{equation*}
		On a donc déterminé notre premier vecteur:
		\begin{equation*}
		...
		Ce système admet plusieurs solutions, on peut choisir celle qui nous arrange.
		Ici, on choisit \(x = 0\), \(y = 1\) et \(z = -1\). On a bien:
		\begin{equation*}
			0 + 2 \cdot 1 + 2 \cdot (-1) = 0.
		\end{equation*}

		Maintenant il faut normaliser \(v'\) pour obtenir \(v\):
		\begin{equation*}
		...
		\begin{equation*}
			\begin{aligned}
				w = u \wedge v &= 
				\frac{1}{3} \cdot \frac{1}{\sqrt{2}} \cdot
				\begin{vmatrix}
					e_1 & e_2 & e_3\\
					1 & 2 & 2\\
					0 & 1 & -1
				\end{vmatrix}\\
				&= \frac{1}{3\sqrt{2}} \cdot e_1
				\begin{vmatrix}
					2 & 2\\
					1 & -1
				\end{vmatrix}
				- \frac{1}{3\sqrt{2}} \cdot e_2
				\begin{vmatrix}
					1 & 2\\
					0 & -1
				\end{vmatrix}
				+ \frac{1}{3\sqrt{2}} \cdot e_3
				\begin{vmatrix}
					1 & 2\\
					0 & 1
				\end{vmatrix}\\
				&= \frac{1}{3\sqrt{2}} \left(e_1(-2 - 2) - e_2(-1 - 0) + e_3(1 - 0)\right)\\
				&= \frac{1}{3\sqrt{2}} (-4, 1, 1).
			\end{aligned}
		\end{equation*}
		On trouve alors notre dernier vecteur:
		\begin{equation*}
			\boxed{w = \frac{1}{3\sqrt{2}}(-4, 1, 1)}.
		\end{equation*}

		\begin{remark}
			Notre dernier vecteur est déjà de norme \(1\) car \(u\) et \(v\) 
			sont orthogonaux et de norme \(1\):
		\end{remark}
...
\iftoggle{showsolutions}{
	\begin{td-sol}[]\ %
		\begin{enumerate}
			\item Si l'équation admet une solution alors par définition 
			du produit vectoriel on a \(v\) orthogonal à \(u\) et \(x\),
			donc en particulier \(v\) est orthogonal à \(u\).

			\item Une solution colinéaire à \(u \wedge v\) s'écrit \(x = \lambda(u \wedge v)\) avec \(\lambda \in \bb R\).
			Calculons \(u \wedge x\):
			\begin{equation*}
				\begin{aligned}
					u \wedge x
			...
			\begin{equation*}
				...
				\end{aligned}
			\end{equation*}
			La solution colinéaire à \(u \wedge v\) est donc:
			\begin{equation*}
				\boxed{x = -\frac{1}{\nn{u}^2}(u \wedge v)}.
			\end{equation*}
